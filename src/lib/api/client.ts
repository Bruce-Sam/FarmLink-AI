...
import { handleAdminDemoRequest } from '@/lib/demo/admin-demo-handlers';
...
export async function apiRequest<T>(
  ...
  if (config.useMockData && normalizedPath.startsWith('/admin')) {
    try {
      const mock = await handleAdminDemoRequest<T>(method, normalizedPath, data);
      if (mock) return mock;
    } catch (error) {
